...
<body>
    <nav class="navbar navbar-expand-lg bg-white fixed-top navbar-scroll">
        <div class="container">
            ...
            <ul class="nav">
                ...
                <li class="nav-item">
                <a class="nav-link active" href="{{ route('sewa.show', ['id' => Auth::user()->id]) }}">My Book</a>
                </li>
                <li class="nav-item">
    ...
    @if ($peminjaman->isEmpty())
        ...
        <div id="{{ $item->idPeminjaman }}" class="card">
            <div>
                @if ($item->buku->gambar_cover)
                <img class="elevation-3 mx-auto" style="width:200px" id="prevImg" src='{{ asset($item->buku->gambar_cover) }}'
                    style="display: block;" />
                ...
                <p class="card-text">Tanggal Peminjaman :{{ $item->tgl_peminjaman }}</p>
                <p class="card-text">Tanggal Pengembalian :{{ $item->tgl_pengembalian }}</p>
